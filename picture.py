...
def detect(filepath, file):
    ...
    # color range
    lower_red1 = np.array([0,100,100])        #define the red channel range in HSV space
    upper_red1 = np.array([10,255,255])
    lower_red2 = np.array([160,100,100])      #define the red channel range in HSV space
    upper_red2 = np.array([180,255,255])
    lower_green = np.array([40,50,50])        #define the green channel range in HSV space
    upper_green = np.array([90,255,255])
    lower_yellow = np.array([15,150,150])     #define the blue channel range in HSV space
    ...
    if DEBUG:
        ...
        cv2.imshow("green", opened_g)
        cv2.imshow("yellow", opened_y)
        cv2.waitKey(0)

    
    ################detect red traffic lighsts########################
    segmentation.clear_border(opened_r)  #eliminate object adjacency to boarder
    label_image_r = measure.label(opened_r)  #label connected area
    borders_r = np.logical_xor(mask_r, opened_r) #xor
    label_image_r[borders_r] = -1
    for region_r in measure.regionprops(label_image_r): #enumerate every connected aread   
        #ignore small and large area	
        if region_r.convex_area < 120 or region_r.area > 2000:
            continue
        #draw bounding box
        area = region_r.area                   #connected area, pixel nums in area
        eccentricity = region_r.eccentricity   #eccentric
        convex_area  = region_r.convex_area    #convex area
        minr, minc, maxr, maxc = region_r.bbox #
        radius = max(maxr-minr,maxc-minc)/2    #longer axis of connected area
        centroid = region_r.centroid           #barycenter coordinate
        perimeter    = region_r.perimeter      #perimeter of area
        x = int(centroid[0])
        y = int(centroid[1])
    ...
    ################detect green traffic lighsts########################
    segmentation.clear_border(opened_g)  #eliminate object adjacency to boarder
    label_image_g = measure.label(opened_g)  #label connected area
    borders_g = np.logical_xor(mask_g, opened_g) #Xor
    label_image_g[borders_g] = -1
    #image_label_overlay_g = color.label2rgb(label_image_g, image=opened_g) #
    for region_g in measure.regionprops(label_image_g): #enumerate every connected area
        #if region_g.convex_area < 130 or region_g.area > 2000:
        #    continue
        area = region_g.area   #area of connected region
        eccentricity = region_g.eccentricity   #eccentric
        convex_area  = region_g.convex_area    #convex area
        minr, minc, maxr, maxc = region_g.bbox #
        radius       = max(maxr-minr,maxc-minc)/2    #longer axis of connected area
        centroid     = region_g.centroid           #barycenter coordinate
        perimeter    = region_g.perimeter      #perimeter of the area
        x = int(centroid[0])
        y = int(centroid[1])
        rect = mpatches.Rectangle((minc, minr), maxc - minc, maxr - minr,
                                  fill=False, edgecolor='red', linewidth=2)
        ...
        if eccentricity <= 0.4 or circularity >= 0.7 or circum_circularity >= 0.8:
            ...
            continue
    '''
    ###############detect yellow traffic lighsts########################
    segmentation.clear_border(opened_y)  #eliminate object adjacency to boarder
    label_image_y = measure.label(opened_y)  #label connected area
    borders_y = np.logical_xor(mask_y, opened_y) #Xor
    label_image_y[borders_y] = -1
    #image_label_overlay_y = color.label2rgb(label_image_y, image=opened_y) #
    for region_y in measure.regionprops(label_image_y): #enumerate every connected are
        if region_y.convex_area < 130 or region_y.area > 2000:
            continue
        area = region_y.area   #area of connected region
        eccentricity = region_y.eccentricity   #eccentric
        convex_area  = region_y.convex_area    #convex area
        minr, minc, maxr, maxc = region_y.bbox #
        radius       = max(maxr-minr,maxc-minc)/2    #longer axis of connected area
        centroid     = region_y.centroid           #barycenter coordinate
        perimeter    = region_y.perimeter      #perimeter of the area
        x = int(centroid[0])
        y = int(centroid[1])
        rect = mpatches.Rectangle((minc, minr), maxc - minc, maxr - minr,
                                  fill=False, edgecolor='red', linewidth=2)
        ...
        if strLight == "NONE":
            ...
            if allLight >=20:
                if (redLight/allLight) >= 0.9:
                    #cmd = raw_input("0xff 01")     #detect red light, send message
                    #sock.sendall(cmd)
                    redLight = 0
                    allLight = 0                     
            ...
            if allLight >= 20:
                if (greenLight/allLight) >= 0.9:
                    #cmd = raw_input("0xff 02")     #detect green light, send message
                    #sock.sendall(cmd)
                    greenLight = 0
                    allLight = 0
            ...
            if allLight >= 20:
                if (yellowLight/allLight) >= 0.9:
                    #cmd = raw_input("0xff 03")     #detect yellow light, send message
                    #sock.sendall(cmd)
                    yellowLight = 0
                    alllight = 0
    #sock.close()        #close socket comm
    #del(capture) 
